...
#load file if exists
destfile <- 'Models/project1_jg_hw_2.RData'
if(file.exists(destfile)){
  load(destfile)
}
if(!file.exists(destfile)){
  #empty metric df
  model_metrics_hw_2_df <- data.frame()
}
...
for(p in seq(6, 24, 6)){
  tmp_ts <- ts(s03_v07_full_ts, frequency = p, start = min(excel_df$SeriesInd))
  for(s in c('additive', 'multiplicative')){
    ...
    model_metrics_hw_2_df <- rbind(model_metrics_hw_2_df, tmp_df)
    print('latest model')
    print(tmp_df %>% arrange(RMSE_100))
    print('all models')
    print(model_metrics_hw_2_df %>% arrange(RMSE_100) %>% head(10))
    flush.console()
  }
}

save(model_metrics_hw_2_df, file = destfile)