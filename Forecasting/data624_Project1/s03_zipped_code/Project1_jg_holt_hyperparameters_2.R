...
#load file if exists
destfile <- 'Models/project1_jg_holt_2.RData'
if(file.exists(destfile)){
  load(destfile)
}
if(!file.exists(destfile)){
  #empty metric df
  model_metrics_holt_2_df <- data.frame()
}
...
for(p in seq(1, 200, 199)){
  ...
  model_metrics_holt_2_df <- rbind(model_metrics_holt_2_df, tmp_df)
  print('latest model')
  print(tmp_df %>% arrange(RMSE_140))
  print('all models')
  print(model_metrics_holt_2_df %>% arrange(RMSE_140) %>% head(10))
  flush.console()
}

save(model_metrics_holt_2_df, file = destfile)