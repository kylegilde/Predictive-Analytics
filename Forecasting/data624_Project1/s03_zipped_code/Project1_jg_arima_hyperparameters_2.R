...
#load file if exists
destfile <- 'Models/project1_jg_arima_2.RData'
if(file.exists(destfile)){
  load(destfile)
}
if(!file.exists(destfile)){
  #empty metric df
 model_metrics_arima_2_df <- data.frame()
}
...
for(pd in seq(90, 210, 30)){
  tmp_ts <- ts(s03_v07_full_ts, frequency = pd, start = 1)
    ...
    for (l in c('NULL', 'auto', 0)){
      for(p in seq(0, 2, 1)){
        for(d in seq(0, 2, 1)){
          ...
          model_metrics_arima_2_df <- rbind(model_metrics_arima_2_df, tmp_df)
          print('latest model')
          print(tmp_df %>% arrange(RMSE_100))
          print('all models')
          print(model_metrics_arima_2_df %>% arrange(RMSE_100) %>% head(10))
          flush.console()
        }
      }
    }
...
save(model_metrics_arima_2_df, file = destfile)
