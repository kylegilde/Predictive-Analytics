...
naive_plot <- autoplot(s4v1_naive_fit) + 
...
naive_plot2 <- autoplot(s4v2_naive_fit) + 
...
stl_mape <- ggplot() + geom_line(data = stl, aes(x=freq,y = mape), color = "red") 
...
stl_plot <- autoplot(s4v1_stl_fit) + 
...
stl_mape2 <- ggplot() + geom_line(data = stl2, aes(x=freq,y = mape), color = "red") 
...
stl_plot2 <- autoplot(s4v2_stl_fit) + 
...
holt_mape <- ggplot() + geom_line(data = holt, aes(x=freq,y = mape), color = "red") 
...
holt_plot <- autoplot(s04_v01_holt_fit) + 
...
holt2 <- do.call(rbind, Map(data.frame, algorithm = 'holt',freq=seq(2,length(rmse)+1), 
                           rmse=rmse, mape=mape))
ggplot() + geom_line(data = holt, aes(x=freq,y = rmse), color = "blue")
holt_mape2 <- ggplot() + geom_line(data = holt, aes(x=freq,y = mape), color = "red") 
...
holt_plot2 <- autoplot(s04_v02_holt_fit) + 
...
hw_mape <- ggplot() + geom_line(data = df, aes(x=freq,y = mape), color = "red") 
...
df <- do.call(rbind, Map(data.frame, algorithm = 'hw',freq=seq(2,length(rmse)+1), 
                         rmse=rmse, mape=mape))
ggplot() + geom_line(data = df, aes(x=freq,y = rmse), color = "blue")
hw_mape2 <-  ggplot() + geom_line(data = df, aes(x=freq,y = mape), color = "red") 
...
arima_ts_plot <- s4v1_ts_NoNA %>% diff() %>% ggtsdisplay(main="")
...
full_data <- ts(s4v1_ts_NoNA, frequency = 1, start = 1)
train_data <- train_datasets[[optimum_freq]]
test_data  <- test_datasets[[optimum_freq]]
...
# As for the other properties, the time plot of the residuals at the top suggests a slight increase in variance over time.
checkresiduals(full_model)
full_fit1 <- full_model %>% forecast(h=prediction_length, level = c(85, 90))

# The forecast appears to be a constant in the prediction window. 
arima_plot <- autoplot(full_fit) + 
  scale_color_manual(values=c('black', 'darkgreen', 'dodgerblue4', 'darkred')) + 
  ggtitle('S04 Var01 ARIMA(17,1,0) Plot') + 
  theme(plot.title = element_text(hjust = 0.5))
...
# Hence, We first check for stationarity of differenced time series. 
# As shown below, the series clearly appear to be stationary after the first differencing. 
# ACF drops to 0 relatively quickly, also suggesting stationarity after first differencing. 
# PACF shows a AR(6) model, but none thereafter, suggesting ARIMA(6,1,1) and ARIMA(6,1,0)


arima_ts_plot2 <- s4v2_ts_NoNA %>% diff() %>% ggtsdisplay(main="")
s4v2_ts_NoNA %>% diff() %>% ggtsdisplay(main="")

mape <- list()
rmse <- list()

# frequencies higher than 30 is "too high" for forecast(ets()) call
# ARIMA(6,1,1) performed slightly better based on AIC. 
(fit <- Arima(s4v2_ts_NoNA, order=c(6,1,1)))
(fit <- Arima(s4v2_ts_NoNA, order=c(6,1,0)))

for (i in 2:30){
  model1 <- Arima(s4v2_ts_NoNA, 
                  order = c(6,1,1),
                  lambda = NULL,
                  include.drift = TRUE)
  rmse <- c(rmse, list(accuracy(model1)[2]))
...
best <- rbind(best, do.call(rbind, Map(data.frame, var='Var02', algorithm = 'arima',
                                       h=optimum_freq, 
                                       rmse=accuracy(test_model)[2], 
                                       mape=accuracy(test_model)[5])))
full_model <- Arima(full_data, 
...
# As for the other properties, the time plot of the residuals at the top suggests a slight increase in variance over time.
checkresiduals(full_model)
full_fit2 <- full_model %>% forecast(h=prediction_length, level = c(85, 90))

# The forecast appears to be a constant in the prediction window. 
arima_plot2 <- autoplot(full_fit) + 
  scale_color_manual(values=c('black', 'darkgreen', 'dodgerblue4', 'darkred')) + 
  ggtitle('S04 Var02 ARIMA(6,1,1) Plot') + 
  theme(plot.title = element_text(hjust = 0.5))
...
grid.arrange(naive_mape, stl_mape, holt_mape, hw_mape, nrow = 2)
grid.arrange(arima_plot, arima_plot2, nrow=2)

write.csv(full_fit1, file = "full_fit1.csv")
write.csv(full_fit2, file = "full_fit2.csv")
