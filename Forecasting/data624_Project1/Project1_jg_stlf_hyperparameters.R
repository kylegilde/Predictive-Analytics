...
#load file if exists
destfile <- 'Models/project1_jg_stlf.RData'
if(file.exists(destfile)){
  load(destfile)
}
if(!file.exists(destfile)){
  #empty metric df
  model_metrics_stlf_df <- data.frame()
}
...
for(p in seq(90, 210, 30)){
  tmp_ts <- ts(s03_v05_full_ts, frequency = p, start = 1)
  ...
  for(m in c('arima', 'ets')){
    for(s1 in c(7, 101, 501)){
      ...
      model_metrics_stlf_df <- rbind(model_metrics_stlf_df, tmp_df)
      print('latest model')
      print(tmp_df %>% arrange(RMSE_140))
      print('all models')
      print(model_metrics_stlf_df %>% arrange(RMSE_140) %>% head(10))
      flush.console()
    }
  }
}

save(model_metrics_stlf_df, file = destfile)
